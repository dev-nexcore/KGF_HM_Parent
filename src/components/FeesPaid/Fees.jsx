        ...
        <div className="w-full bg-white rounded-2xl shadow-inner border border-gray-100 overflow-hidden" style={{ boxShadow: 'inset 0 4px 10px rgba(0, 0, 0, 0.1)' }}>
          <div className="w-full flex flex-col items-center p-4 md:p-5 pb-4">
            ...
            <div className="md:hidden w-full max-w-sm">
              ...
              {/* Mobile Data Cards - All text made bold */}
              <div className="space-y-3 mb-0">
                {feesData.map((row, i) => (
                  <div
                    key={i}
                    className={`bg-white border border-gray-200 p-5 rounded ${i === feesData.length - 1 ? 'mb-0' : ''}`}
                  >
                    <div className="grid grid-cols-4 gap-3 text-sm items-center">
                      ...
                      <div className="flex justify-center items-center py-1">
                        <span
                          className={`flex items-center justify-center w-[70px] h-[32px] text-xs font-semibold rounded-sm ${statusColors[row.status]}`}
                        >
                          {row.status}
                        </span>
                      </div>
                    </div>
                    {/* Payment Date shown below in mobile - made bold */}
                    <div className="mt-3 pt-3 border-t border-gray-200">
                      <div className="text-center">
                        <span className="text-xs font-bold text-gray-500">Payment Date: </span>
                        <span className="text-sm font-bold text-gray-700">{row.paid}</span>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="md:hidden w-full max-w-sm">
              ...
              <div className="space-y-3 mb-0">
                {transactionData.map((row, i) => (
                  <div
                    key={i}
                    className={`bg-white border border-gray-200 p-5 rounded ${i === transactionData.length - 1 ? 'mb-0' : ''}`}
                  >
                    <div className="grid grid-cols-4 gap-3 text-sm items-center">
                      <div className="text-center font-bold text-gray-800 py-1">{row.id}</div>
                    ...
                    {/* Method shown below in mobile - made bold */}
                    <div className="mt-3 pt-3 border-t border-gray-200">
                      <div className="text-center">
                        <span className="text-xs font-bold text-gray-500">Method: </span>
                        <span className="text-sm font-bold text-gray-700">{row.method}</span>
                      </div>
                    </div>
                  </div>
                ))}
